  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-primary">
        <div className="container flex items-center justify-center gap-10 py-2 text-xs text-primary-foreground/90">
          <a
            href="tel:8006212178"
      ...
      <div className="bg-card border-b border-border shadow-sm">
        <div className="container flex items-center gap-6 py-3">
          ...
          {/* Logo */}
          <Link to="/v3" className="shrink-0 w-44">
          ...
          <div className="flex-1 flex justify-center">
            <div className="w-full max-w-2xl">
              <div className="flex items-center w-full rounded border border-input bg-card overflow-hidden focus-within:ring-2 focus-within:ring-primary focus-within:border-primary">
                <input
                  type="text"
                  placeholder="Search by product name or item number"
                  value={searchValue}
                  onChange={(e) => setSearchValue(e.target.value)}
                  className="flex-1 h-10 pl-4 pr-3 bg-transparent text-sm focus:outline-none text-foreground placeholder:text-muted-foreground"
                />
                <button className="px-4 h-10 flex items-center text-muted-foreground hover:text-primary transition-colors border-l border-input">
          ...
          {/* Account + Cart */}
          <div className="hidden sm:flex items-center gap-4 shrink-0 w-44 justify-end">
            {/* Account */}
            <button className="flex items-center gap-2 text-sm text-foreground hover:text-primary transition-colors">
              <span className="w-7 h-7 rounded-full bg-primary text-primary-foreground text-[11px] font-bold flex items-center justify-center shrink-0">
                SD
              </span>
              <span className="font-medium whitespace-nowrap">Safco Dental</span>
      ...
      <nav className="hidden lg:block bg-card border-b border-border">
        <div className="container flex items-center gap-1 py-0.5">
          {NAV_ITEMS.map((item) =>
            item.isVersionPicker ? (
              <div key={item.label} className="relative" ref={versionDropdownRef}>
                <button
                  onClick={() => setVersionDropdownOpen((v) => !v)}
                  className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-semibold text-foreground border border-border rounded hover:border-primary hover:text-primary transition-colors whitespace-nowrap"
                >
                  {item.label}
                  <ChevronDown
                    className={`h-3.5 w-3.5 text-muted-foreground transition-transform duration-200 ${versionDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {versionDropdownOpen && (
                  <div className="absolute left-0 top-full mt-1 w-48 bg-card border border-border rounded shadow-lg overflow-hidden z-50">
                    <Link
                      to="/"
          ...
          <Link
            ...
            className="px-3 py-2.5 text-sm font-medium text-foreground hover:text-primary hover:bg-muted/60 rounded transition-colors whitespace-nowrap ml-2"
          >
            Quick Order
          </Link>
        ...
        <div className="lg:hidden border-t border-border bg-card">
          <div className="flex flex-col">
            ...
            <Link
              to="/category/personalized"
              onClick={() => setMobileMenuOpen(false)}
              className="px-4 py-3 text-sm font-medium text-primary border-b border-border hover:bg-muted flex items-center gap-2"
            >
